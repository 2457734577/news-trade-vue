  ...
  <div class="asset-card" :class="`asset-card-${exchangeType.toLowerCase()}`">
    ...
    <div class="card-header">
      <div class="exchange-info">
        ...
        <div class="exchange-details">
          <h3 class="exchange-name">{{ account.exchangeName || getExchangeName(exchangeType) }}</h3>
          <span class="account-label">账户ID: {{ account.accountId }}</span>
        </div>
      </div>
    ...
    <div class="total-asset">
      ...
      <div class="asset-value">
        ...
        <span class="amount">{{ formatCurrency(account.totalBalance) }}</span>
      </div>
      <div 
    ...
    <!-- 合约可用余额 -->
    <div v-if="account.futuresAvailable" class="margin-info">
      <div class="margin-item">
        <span class="margin-label">合约可用</span>
        <span class="margin-value available">${{ formatCurrency(account.futuresAvailable) }}</span>
      </div>
      <div class="margin-item">
        <span class="margin-label">未实现盈亏</span>
        <span 
          class="margin-value"
          :class="account.unrealizedPnl >= 0 ? 'positive' : 'negative'"
        >
    ...
    <!-- 详细资产列表（现货资产） -->
    <div v-if="expanded && spotAssetsList.length > 0" class="asset-details">
      ...
      <div class="details-list">
        <div v-for="asset in spotAssetsList" :key="asset.coin" class="asset-item">
    ...
    <!-- 最后更新时间 -->
    <div v-if="account.updateTime" class="update-time">
...
/**
 * 现货资产列表（从spotAssets对象转换为数组）
 */
const spotAssetsList = computed(() => {
  if (!props.account.spotAssets) return []
  
  return Object.entries(props.account.spotAssets).map(([coin, amount]) => ({
    coin,
    amount,
    usdtValue: amount // 简化处理，实际应该根据价格计算
...
async function refreshAsset() {
  ...
  try {
    await assetsStore.refreshAccountBalance(props.account.accountId)
  } finally {
